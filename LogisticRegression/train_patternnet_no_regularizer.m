function [Y,NErrors,W] = train_patternnet_no_regularizer(X, T, num_epochs)
    ...
    assert(all((sum(T,1)-1) < 10*eps),'Target rows must sum up to 1');
    ...
    G = loss(W,Y,T)         % Test on the original sample
    Gn = [G];
